 function sendFile(e) {
	...
	fr.onload = ()=>{
		...
		//console.log(String(sendfile))
		fetch('/predict',{
			'method':'POST',
			'headers': {
				'Content-Type': 'application/json',
			  },
			'body': JSON.stringify(JSON.parse(sendfile))
		}).then((res)=>{ return res.json()}).then((data)=>{console.log(data);putInTable(data) });
	}
 ...
 function putInTable(data) {
	 data.forEach(ele => {
		 let times = ele.Time.split(',');
		 console.log('eleet tie',ele,times)
			times.forEach(time=>{
				if (time === '') return;
				let slot = '';
				switch (time) {
				...
				addCourse(ele.Subject,slot,ele.Classroom);
